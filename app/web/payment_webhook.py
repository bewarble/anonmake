...
async def _operation_id(payload: WebhookPayload, request: Request) -> str:
    ...
    if not operation_id:
        ...
        operation_id = raw.get("customer_operation_id") or transaction.get("customer_operation_id")
    if not operation_id:
        raise HTTPException(status_code=422, detail="customer_operation_id is required")
    return str(operation_id)


async def _owner_for_operation(session, operation_id: str) -> tuple[int, RevealCheckout | None]:
    checkout = await session.scalar(select(RevealCheckout).where(RevealCheckout.customer_operation_id == operation_id))
    if checkout is not None:
        bot_id = await session.scalar(select(User.bot_id).where(User.id == checkout.buyer_id))
        if bot_id is None:
            raise HTTPException(status_code=409, detail="Reveal checkout owner is missing")
        return int(bot_id), checkout
    bot_id = await session.scalar(select(PaymentAttempt.bot_id).where(PaymentAttempt.customer_operation_id == operation_id))
    if bot_id is None:
...
async def impaya_webhook(payload: WebhookPayload, request: Request, x_webhook_secret: str | None = Header(default=None)) -> dict[str, str]:
...
def install_impaya_webhook(app) -> None:
    if getattr(app.state, "project_impaya_webhook_installed", False):
        return
    app.router.routes[:] = [route for route in app.router.routes if not (getattr(route, "path", None) == WEBHOOK_PATH and "POST" in (getattr(route, "methods", None) or set()))]
    app.add_api_route(WEBHOOK_PATH, impaya_webhook, methods=["POST"], include_in_schema=True)
