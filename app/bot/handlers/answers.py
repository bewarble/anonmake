...
async def begin_answer(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
...
async def begin_answer_back(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    await state.update_data(recipient_id=question.recipient_id, reply_mode=True)
    await callback.answer()
    if callback.message:
        await callback.message.answer(texts.ANSWER_PROMPT, reply_markup=cancel_keyboard())
...
async def receive_answer(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await DeliveryRepository(session).enqueue(
        ...
        text=texts.ANSWER_RECEIVED.format(answer=html.escape(text)),
    ...
    await message.answer(texts.ANSWER_SENT, reply_markup=answer_share_keyboard(personal_link))
