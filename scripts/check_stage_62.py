...
def main() -> None:
    ...
    require(
        "app/services/reveal_checkout.py",
        'checkout.status == "payment_pending" and checkout.invoice_id',
        "current_bot = require_current_bot()",
        "def payment_url",
    )
    # The accidental duplicate assignment found during launch audit must stay gone.
    ...
    require(
        "app/core/config.py",
        "payment_test_commands_enabled: bool = Field(default=False",
    )
